...
function App() {
  ...
  const editEvent = (updates, eventId) => {
    axios
      .put(`http://localhost:8000/events/${eventId}`, updates)
      .then((res) => {
        setEvents((prevEvents) =>
          prevEvents.map((event) => (event._id !== eventId ? event : res.data))
        );
      })
      .catch((err) => console.log(err));
  };
  ...
  return (
    <div>
      <h1 className='title'>My Upcoming Events</h1>
      <AddEvents submit={addEvent} btnText='Add Event' />
      {events.map((event) => {
        return (
          <Events
            {...event}
